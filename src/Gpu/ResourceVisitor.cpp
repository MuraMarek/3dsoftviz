...
void Gpu::ResourceVisitor::distribute(osg::Node& node)
{
	...
	if(NULL != dataNode && _vertexBuffer != NULL && _vertexOffsets->contains(dataNode->getId()))
	{
		...
		// PK fix - docasne zakomentovane, lebo neni implementovana setTargetPositionPtr()
		...
		// PK fix - docasne zakomentovane, lebo neni implementovana setGpuFlagsPtr()
...
void Gpu::ResourceVisitor::addPositionResource()
{
	if(_vertexBuffer == NULL && _nodeCount > 0)
	{
		...
		//vertex positions
		_vertexBuffer = new osgCuda::Buffer;
...
//		}
		...
		//vertex velocities
		osg::ref_ptr<osgCompute::Memory> velocityBuffer = new osgCuda::Buffer;
...
//		}
		...
		//edges
		osg::ref_ptr<osgCompute::Memory> edgeBuffer = new osgCuda::Buffer;
